...
CXChildVisitResult handle_generic_cursor(CXCursor cursor, code* current_scope) {
	...
	*/
	...
	if (
		...
		if (mutates) {
			handle_generic_cursor_children(cursor, current_scope);
			current_scope->points.back().dependency.push_back(unary_node.dependency[0]);
			current_scope->points.back().mutated = true;
		}

		auto unary_index = current_scope->points.size();
		current_scope->flows[scope].local_parameters.push_back(current_scope->points.size());
...
bool print_point (code& code_db, std::map<std::string, std::string>& result, point& item, bool left_part, std::string prefix) {
	...
	if (item.computation.size() > 0) {
		...
		for (auto part_index : item.computation) {
			...
			result_string += ninja_name(part.name) +  "_";
		}
	}
	if (left_part)
...
void print_flow (code& code_db, std::map<std::string, std::string>& result, flow& flow, std::vector<int> flow_chain, std::vector<std::string> prefix_chain, std::map<int, int>& substitution){
	...
	for (auto item_index : flow.local_parameters) {
		auto& item = code_db.points[item_index];
		...
		for (auto dep_index : item.dependency) {
			...
			print_point(code_db, result, dep, false, prefix_chain[actual_layer]);
			std::cout << " -> ";
			print_point(code_db, result, item, true, prefix_chain[actual_layer]);
			std::cout << "\n";
		}
		...
		if (item.is_function_call) {
			...
			while(there_is_a_way) {
				...
				} else if (substitution.find(current) != substitution.end()) {
					there_is_a_way = true;
			...
			print_flow(code_db, result, next_flow, flow_chain, prefix_chain, next_substitution);
...
int main(){
	...
	for (auto& flow : code_db.flows) {
		...
		// if (flow.name[2] == 'e' && flow.name[3] == 'c' && flow.name[4] == 'o')
			print_flow(code_db, result, flow, {0, i}, {"ROOT", ninja_name(flow.name)}, sub);
		i++;
	}
